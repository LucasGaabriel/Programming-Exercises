...
void ordenaPalavrasEncontradas(int pPorJ, int p[], int t[], char palavrasEncontradas[][MaxLetras]){
    ...
    for(i=0; i<pPorJ*2; i++){	//CORRECAO: Estava contando a quantidade de palavras que nao foram descobertas (com o numero 0), que e desnecessario)
       	if(p[i] == 1)
            qtd1++;
        else if(p[i] == 2)
            qtd2++;
    }
		//CORRECAO: Nao estava ordenando corretamente as palavras (Estava usando if(strcmp() == 1), quando o correto e if(strcmp() > 0) )
    //Ordena alfabeticamente as palavras com Pontuacao '2'
    for(i=0; i<qtd2; i++){
        for(j=0; j<qtd2-1; j++){
            if(strcmp(palavrasEncontradas[j], palavrasEncontradas[j+1]) > 0){
                strcpy(aux2, palavrasEncontradas[j]);
                strcpy(palavrasEncontradas[j], palavrasEncontradas[j+1]);
                strcpy(palavrasEncontradas[j+1], aux2);
    ...
    for(i=qtd1+qtd2; i<(pPorJ*2); i++){   
        for(j=qtd1+qtd2; j<(pPorJ*2)-1; j++){
            if(strcmp(palavrasEncontradas[j], palavrasEncontradas[j+1]) > 0){
                strcpy(aux2, palavrasEncontradas[j]);
                strcpy(palavrasEncontradas[j], palavrasEncontradas[j+1]);
                strcpy(palavrasEncontradas[j+1], aux2);
		...
		//CORRECAO: Todo esse trecho de ordenacao estava incorreto (usando valor do retorno incorreto)
}
...
void exibeInfo(int tamM, int pPorJ, char mChar[][N], char palavras1[][MaxLetras], char palavras2[][MaxLetras], char matrizCP[][N], tJogador jogador1, tJogador jogador2){
    ...
    for(i=0; i<tamM; i++){
        if(i != tamM-1)
            ...
            printf("%02d", i);	//CORRECAO: No ultimo numero estava escrevendo um espaco em branco. Foi adicionado um if/else para quando for imprimir o ultimo numero, nao coloque espaco em branco
    }
    printf("\n");
...
void removeEspaco(char iniciais[], int pPorJ){
    ...
    //CORRECAO: Mudanca no Algoritmo para Remover os Espacos. O algoritmo anterior nao removia espacos quando entre uma palavra e outra havia muitos espacos.
    for(i=0, j=0; i<pPorJ; i++){
        if(iniciais[i] >= 'a' && iniciais[i] <= 'z'){
            iniciais[j] = iniciais[i];
            j++;
        }
    }
}
...
void arquivoInicializacao(tJogador jogador1, tJogador jogador2, char palavras1[][MaxLetras], char palavras2[][MaxLetras], int pPorJ, char **argv){
    ...
    //CORRECAO: Foram removidos/adicionados espacos em branco necessarios/desnecessarios
}
